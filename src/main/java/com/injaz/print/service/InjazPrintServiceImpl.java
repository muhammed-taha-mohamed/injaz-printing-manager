...
public class InjazPrintServiceImpl implements InjazPrintService  {
	...
	public void printKitchenReceipt(InjazPrintOrderRestModel order) throws Exception {
        ...
        PrintService printService = PrinterOutputStream.getPrintServiceByName(order.getPrinter());
         ...
         escpos.write(imageWrapper,new EscPosImage(getKitchenHeaderTextImage(env.getProperty("DATE_TIME_LABEL")+" "+ (order.getOrderDate()!=null ? order.getOrderDate() : InjazUtility.getInstance().convertInstantIntoString(Instant.now(),InjazAppConstants.DATE_FORMAT_RECEIPT_PRINT))), algorithm));
	...
	public void printOrderReceipt(InjazPrintOrderRestModel order) throws Exception {
         ...
         //date
         imageWrapper.setJustification(EscPosConst.Justification.Center);
         escpos.write(imageWrapper,new EscPosImage(getFooterWithoutBoldTextImage(env.getProperty("DATE_TIME_LABEL")+" "+(order.getOrderDate()!=null ? order.getOrderDate() : InjazUtility.getInstance().convertInstantIntoString(Instant.now(),InjazAppConstants.DATE_FORMAT_RECEIPT_PRINT))), algorithm));
    ...
    @Override
    public void rePrintOrderReceipt(InjazPrintOrderRestModel order) throws Exception {
